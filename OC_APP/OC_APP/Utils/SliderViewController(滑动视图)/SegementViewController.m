...
@interface SegementViewController ()<UICollectionViewDataSource,UICollectionViewDelegate> {
    UIColor *_norColor;
    UIColor *_selColor;
}
/** 下标宽度是否等于标题宽度 */
...
- (void)viewDidLayoutSubviews {
    ...
    if (_isInitial == NO) {
        ...
        CGFloat titleY = self.navigationController.navigationBarHidden == NO ? 0 : statusH;
        // 是否占据全屏
        if (_isFullScreen) {
            // 整体contentView尺寸
            self.contentView.frame = CGRectMake(0, 0, kScreenWidth, kScreenHeight);
            // 顶部标题View尺寸
            self.titleScrollView.frame = CGRectMake(0, titleY, kScreenWidth, self.titleHeight);
            // 顶部内容view尺寸
            self.contentScrollView.frame = self.contentView.bounds;
            return;
        }
        if (self.contentView.frame.size.height == 0)
            self.contentView.frame = CGRectMake(0, titleY, kScreenWidth, kScreenHeight - titleY - kNavHeight);
        NSLog(@"%f", kNavHeight);
        NSLog(@"%f", kNavBarHeight);
        ...
        CGFloat contentH = self.contentView.xl_height - contentY;
        self.contentScrollView.frame = CGRectMake(0, contentY, kScreenWidth, contentH);
...
- (void)setupTitleWidth {
    ...
    for (NSString *title in titles) {
        if ([title isKindOfClass:[NSNull class]]) {
            NSException *excp = [NSException exceptionWithName:@"SegementViewControllerException" reason:@"没有设置Controller.title属性，应该把子标题保存到对应子控制器中" userInfo:nil];
            [excp raise];
        }
...
- (void)setupAllTitle {
    ...
    for (NSInteger i = 0; i < count; i++) {
        ...
        UILabel *label = [[TitleLabel alloc] init];
        label.tag = i;
...
- (void)setupTitleColorGradientWithOffset:(CGFloat)offsetX rightLabel:(TitleLabel *)rightLabel leftLabel:(TitleLabel *)leftLabel {
    if (_isShowTitleGradient == NO) return;
    // 获取右边缩放比例
    CGFloat rightSacle = offsetX / kScreenWidth - leftLabel.tag;
    // 获取左边缩放比例
...
/** 标题缩放 */
- (void)setupTitleScaleWithOffset:(CGFloat)offsetX rightLabel:(TitleLabel *)rightLabel leftLabel:(TitleLabel *)leftLabel {
...
- (void)setupUnderLineOffset:(CGFloat)offsetX rightLabel:(UILabel *)rightLabel leftLabel:(UILabel *)leftLabel {
    ...
    // 获取两个标题中心点距离
    CGFloat centerDelta = rightLabel.xl_x - leftLabel.xl_x;
    //标题宽度差
    ...
    self.underLine.xl_width += underLineWidth;
    self.underLine.xl_x += underLineTransformX;
}
...
- (void)setupCoverOffset:(CGFloat)offsetX rightLabel:(UILabel *)rightLabel leftLabel:(UILabel *)leftLabel {
    ...
    // 计算当前下划线偏移量
    CGFloat coverTransformX = offsetDelta * centerDelta / kScreenWidth;
    
    // 宽度递增偏移量
    CGFloat coverWidth = offsetDelta * widthDelta / kScreenWidth;
    
    self.coverView.xl_width += coverWidth;
    self.coverView.xl_x += coverTransformX;
}
...
- (void)setSelectIndex:(NSInteger)selectIndex {
    ...
    if (self.titleLabels.count) {
        ...
        if (_selectIndex >= self.titleLabels.count) {
            @throw [NSException exceptionWithName:@"SegementView-Error" reason:@"选中控制器的角标越界" userInfo:nil];
        }
...
- (void)selectLabel:(UILabel *)label {
    for (TitleLabel *labelView in self.titleLabels) {
        ...
        if (_isShowTitleGradient) labelView.transform = CGAffineTransformIdentity;
    ...
    // 设置下标的位置
    if (_isShowUnderLine) [self setupUnderLine:label];
    // 设置cover
    if (_isShowTitleCover) [self setupCoverView:label];
}
...
- (void)setupUnderLine:(UILabel *)label {
    ...
    self.underLine.xl_y = label.xl_height - underLineH;
    self.underLine.xl_height = underLineH;
    
    
    // 最开始不需要动画
    if (self.underLine.xl_x == 0) {
        if (_isUnderLineEqualTitleWidth) {
            self.underLine.xl_width = titleBounds.size.width;
        } else {
            self.underLine.xl_width = label.xl_width;
        }
        
        self.underLine.xl_centerX = label.xl_centerX;
        return;
    }
...
- (void)setupCoverView:(UILabel *)label {
    ...
    self.coverView.xl_y = (label.xl_height - coverH) * 0.5;
    self.coverView.xl_height = coverH;
    
    
    // 最开始不需要动画
    if (self.coverView.xl_x == 0) {
        self.coverView.xl_width = coverW;
        
        self.coverView.xl_x = label.xl_x - border;
        return;
    }
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    vc.view.frame = CGRectMake(0, 0, self.contentScrollView.xl_width, self.contentScrollView.xl_height);
    
    CGFloat bottom = self.tabBarController == nil ? 0 : 0;
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView {
    ...
    NSInteger screenWInt = kScreenWidth;
    
    NSInteger extre = offsetXInt % screenWInt;
    if (extre > kScreenWidth * 0.5) {
        // 往右边移动
        offsetX = offsetX + (kScreenWidth - extre);
        _isAniming = YES;
        [self.contentScrollView setContentOffset:CGPointMake(offsetX, 0) animated:YES];
    }else if (extre < kScreenWidth * 0.5 && extre > 0){
        _isAniming = YES;
        // 往左边移动
...
- (void)setIsShowTitleScale:(BOOL)isShowTitleScale {
    if (_isShowUnderLine) {
        // 抛出异常
        NSException *excp = [NSException exceptionWithName:@"SegementViewControllerException" reason:@"字体放大效果和角标不能同时使用！" userInfo:nil];
        [excp raise];
    }
...
- (void)setIsFullScreen:(BOOL)isFullScreen {
    ...
    self.contentView.frame = CGRectMake(0, 0, kScreenWidth, kScreenHeight);//这是为了调用viewDidLayoutSubviews.
}

- (void)setupContentViewFrame:(void (^)(UIView *))contentBlock {
    if (contentBlock) contentBlock(self.contentView);
}

- (void)setupTitleGradient:(void (^)(TitleColorGradientStyle *, UIColor *__autoreleasing *, UIColor *__autoreleasing *))titleGradientBlock {
    ...
    if (_titleColorGradientStyle == TitleColorGradientStyleFill && _titleWidth > 0) {
        @throw [NSException exceptionWithName:@"SegementView-Error" reason:@"标题颜色填充不需要设置标题宽度" userInfo:nil];
    }
}

- (void)setupCoverEffect:(void (^)(UIColor *__autoreleasing *, CGFloat *))coverEffectBlock {
    UIColor *color;
    _isShowTitleCover = YES;
    if (coverEffectBlock) {
        coverEffectBlock(&color, &_coverCornerRadius);
        if (color) _coverColor = color;
    }
}
...
- (void)setupTitleScale:(void (^)(CGFloat *))titleScaleBlock {
    ...
    if (_isShowUnderLine) {
        @throw [NSException exceptionWithName:@"SegementView-Error" reason:@"当前框架下标和字体缩放不能一起用" userInfo:nil];
    }
    if (titleScaleBlock) titleScaleBlock(&_titleScale);
}

- (void)setupUnderLineEffect:(void (^)(BOOL *, CGFloat *, UIColor *__autoreleasing *, BOOL *))underLineBlock {
...
- (void)setupTitleEffect:(void (^)(UIColor *__autoreleasing *, UIColor *__autoreleasing *, UIColor *__autoreleasing *, UIFont *__autoreleasing *, CGFloat *, CGFloat *))titleEffectBlock {
    ...
    if (titleEffectBlock) {
        ...
        if (norColor) self.norColor = norColor;
        if (selColor) self.selColor = selColor;
        if (titleScrollViewColor) _titleScrollViewColor = titleScrollViewColor;
...
- (UICollectionView *)contentScrollView {
    if (!_contentScrollView) {
        SegementViewFlowLayout *layout = [[SegementViewFlowLayout alloc] init];
