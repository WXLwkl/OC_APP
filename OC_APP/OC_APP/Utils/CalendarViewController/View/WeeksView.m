...
- (NSMutableArray*)labsArray {
    if (!_labsArray) {
        _labsArray = [NSMutableArray arrayWithCapacity:7];
    }
    return _labsArray;
}
- (UIButton *)lastButton {
    if (!_lastButton) {
        ...
        [_lastButton addTarget:self action:@selector(buttonClick:) forControlEvents:UIControlEventTouchUpInside];
