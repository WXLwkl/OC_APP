...
- (void)badgeInit
{
    if (nil == self.lf_badge) {
        ...
        self.lf_badge.backgroundColor = RGBColor(228,32,38);
        self.lf_badge.textColor = [UIColor whiteColor];
        self.lf_badge.text = @"";
        self.lf_badge.layer.cornerRadius = self.lf_badge.xl_width / 2;
...
- (void)configBadge:(LFBadgeType)type style:(LFBadgeStyle)style sizeType:(LFBadgeSizeType)sizeType value:(NSInteger)value{
    ...
    switch (type) {
        ...
        {
            self.lf_badge.text = @"";
            self.lf_badge.xl_width = LF_BADGE_VIEW_REDDOT_WIDTH;
            self.lf_badge.xl_height = LF_BADGE_VIEW_REDDOT_WIDTH;
        }
            break;
        case LFBadgeTypeNumber:
        {
            ...
            if(value >= 100){
                self.lf_badge.xl_width = LF_BADGE_VIEW_BIG_WIDTH;
            }
            else if(value >= 10){
                self.lf_badge.xl_width = LF_BADGE_VIEW_STANDARD_WIDTH;
            }
            else{
                self.lf_badge.xl_width = LF_BADGE_VIEW_SAMLL_WIDTH;
            }
            self.lf_badge.xl_height = LF_BADGE_VIEW_HEIGHT;
    ...
    if(style == LFBadgeStyleNormal){
        self.lf_badge.center = CGPointMake(self.xl_width, 0);
    }
    else if(style == LFBadgeStyleCircle){
        CGFloat radius = self.xl_width/2;
        self.lf_badge.center = CGPointMake(ceil(radius + sqrt(2)/2*radius), ceil(radius - sqrt(2)/2*radius));
    }
    
    //*********自己定义的X Y*************//
    if([self hasBadgeOriginX]){
        self.lf_badge.xl_x = self.lf_badgeOriginX;
    }
    
    if([self hasBadgeOriginY]){
        self.lf_badge.xl_y = self.lf_badgeOriginY;
    }
    //********************************//

    self.lf_badge.layer.cornerRadius = self.lf_badge.xl_height / 2;
...
- (UILabel *)lf_badge
{
    ...
    if(lbl == nil) {
        ...
        lbl.backgroundColor = RGBColor(228,32,38);
...
-(void) setLf_badgeOriginX:(CGFloat)badgeOriginX
{
    ...
    self.lf_badge.xl_x = badgeOriginX;
}
...
-(void) setLf_badgeOriginY:(CGFloat)badgeOriginY
{
    ...
    self.lf_badge.xl_y = badgeOriginY;
}
