...
@implementation DrawerTransition
{
    DrawerTransitiontype _transitionType;
    DrawerAnimationType _animationType;
}
...
- (NSTimeInterval)transitionDuration:(nullable id <UIViewControllerContextTransitioning>)transitionContext {
    return _transitionType == DrawerTransitiontypeShow ? 0.4f : 0.25f;
}
- (void)animateTransition:(id <UIViewControllerContextTransitioning>)transitionContext {
    switch (_transitionType) {
        case DrawerTransitiontypeShow:
            [self animationViewShow:transitionContext];
            break;
        case DrawerTransitiontypeHidden:
...
- (void)animationViewShow:(id <UIViewControllerContextTransitioning>)transitionContext {
    if (_animationType == DrawerAnimationTypeDefault) {
        [self defaultAnimationWithContext:transitionContext];
    } else if (_animationType == DrawerAnimationTypeMask) {
...
- (void)defaultAnimationWithContext:(id <UIViewControllerContextTransitioning>)transitionContext {
    ...
    CGFloat x = - width / 2;
    CGFloat ret = 1;
    if (self.configuration.direction == DrawerTransitionDirectionRight) {
...
- (void)singleTap {
    [[NSNotificationCenter defaultCenter] postNotificationName:LateralSlideTapNotication object:self];
}

- (void)handleGesture:(UIPanGestureRecognizer *)pan {
    [[NSNotificationCenter defaultCenter] postNotificationName:LateralSlidePanNotication object:pan];
    
}
@end

NSString *const LateralSlideMaskViewKey = @"LateralSlideMaskViewKey";
NSString *const LateralSlideAnimatorKey = @"LateralSlideAnimatorKey";
NSString *const LateralSlideInterativeKey = @"LateralSlideInterativeKey";

NSString *const LateralSlidePanNotication = @"LateralSlidePanNotication";
NSString *const LateralSlideTapNotication = @"LateralSlideTapNotication";
