...
@implementation AssetWriteManager
...
- (void)setup {
    ...
    _writeQueue = dispatch_queue_create("com.xingl.capture", DISPATCH_QUEUE_SERIAL);
    _recordTime = 0;
}

#pragma mark - public Method
- (void)startWrite {
    self.writeState = LLRecordStatePrepareRecording;
    if (!self.assetWriter) {
        [self setupWriter];
    }
}
- (void)stopWrite {
    self.writeState = LLRecordStateFinish;
...
- (void)appendSampleBuffer:(CMSampleBufferRef)sampleBuffer ofMediaType:(NSString *)mediaType {
    ...
    if (sampleBuffer == NULL) return NSLog(@"empty sampleBuffer");
    
    @synchronized(self){
        if (self.writeState < LLRecordStateRecording) return NSLog(@"not ready yet");
    }
    ...
    dispatch_async(self.writeQueue, ^{
        @autoreleasepool {
            @synchronized(self) {
                if (self.writeState > LLRecordStateRecording) return CFRelease(sampleBuffer);
            }
...
- (void)setupWriter {
    ...
    self.writeState = LLRecordStateRecording;
}
